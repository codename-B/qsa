...
Second, the prediction dot product uses 32-bit arithmetic that wraps. Thus
any integer decoder agrees with this reference bit for bit. The file does not
hold the filter state, so a decoder must start at the first chunk.
